...
工作台新增一组由同一人物、服装、旧剧院编剧室和核心道具贯穿的 50 张故事位图。页面默认只呈现 20 张核心流程画面，导入原文、生成路线、选择确认、下载交付和保存反馈时，会自动切换到对应阶段；使用者也可以展开完整 50 幕逐项复核。

- `public/story-v3/manifest.json` 是 50 条人物、场景、动作、产品状态、结果、替代文本与具体生成提示词的唯一清单。
- `src/lib/story-studio/visual-story-v3.ts` 是产品直接引用的运行时记录；20 条核心记录由清单明确标记，不靠文件名推断。
- `npm run visual-story:build` 从经审阅的数据源重建清单与运行时记录，`npm run visual-story:validate` 校验数量、唯一哈希、位图格式、真实引用与核心可达性。
- `docs/visual-v3-qa-checklist.md` 记录人工拼图检查、390px 真实解码、无障碍、性能和发布产物验收口径。
