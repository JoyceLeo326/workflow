...
- 无 Provider 时只运行确定性本地规则流程，并明确标注“本地规则演示，不是 AI 生成”。
...
本仓库的 Vercel 配置只面向个人、非商业作品演示。Vercel Hobby 条款明确限制为个人或非商业用途，且平台可调整限制；商业使用前必须重新评估计划与合同：[Vercel Terms of Service](https://vercel.com/legal/terms)。公开演示默认使用浏览器本地规则，不依赖持久文件系统或付费 Provider。

### Cloudflare Pages / Workers

静态展示可评估 Cloudflare Pages；需要函数时，Pages Functions 按 Workers 计量。当前 Next.js API 路由与本地文件存储不能直接假定兼容，迁移前必须选择官方支持的适配或拆分为静态前端与 Workers API，并继续执行 fail-closed 配额策略。
...
商业化应由运营主体在自己的云账号或服务器上自托管，使用机构 Provider、持久数据库和权威计量。上线前必须设置硬预算、告警、限流、逐能力开关和人工审批；这条路线不属于“项目所有者零固定成本”的个人演示承诺，费用与责任由商业运营主体承担。
